...
public class HighScores extends JFrame implements ActionListener, Serializable{
	...
	//didGameEnd- tells us if the game ended. if it did, closes the game after closing this frame
	...
	private final int DEFAULT= 999999999;
	//OK- the "ok" button
	...
	public HighScores (){
		...
		addWindowFocusListener(this.windowFocusListener);
	...
	public void changeHighScore(int newscore, String name){
		...
		try {	//after updating the table, saves it to file
			FileOutputStream fileOutputStream = new FileOutputStream(Game.HIGHSCORES_FILE);
	...
	public void closeThis(){
		if (this.didGameEnd){	//if this table was shown after the game ended- closes the game
			StartMenu rerunGame= new StartMenu(null);
		}
		dispose();
	}//closeThis()
